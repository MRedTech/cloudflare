...
async function markSyncFailed(env, recordId, message) {
  try {
    await dbRun(env, "UPDATE entries SET sync_status = 'FAILED', sync_error = ? WHERE id = ?", cleanSyncError(message), recordId);
  } catch (_) {}
}
...
async function syncToGoogle(env, record) {
  ...
  if (!env.GAS_SYNC_URL) {
    await markSyncFailed(env, record.id, "GAS_SYNC_URL is missing");
    return;
  }
  if (!env.SYNC_TOKEN) {
    await markSyncFailed(env, record.id, "SYNC_TOKEN is missing");
    return;
  }
  ...
  for (let attempt = 1; attempt <= tries; attempt++) {
    try {
      // Count each real sync attempt.
      try {
        await dbRun(env, "UPDATE entries SET sync_attempts = COALESCE(sync_attempts,0) + 1 WHERE id = ?", record.id);
      } catch (_) {}
  ...
  await markSyncFailed(env, record.id, lastError);
}

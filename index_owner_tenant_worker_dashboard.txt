...
//   imageUrl          // optional only when an active previous photo exists
//
// IMPORTANT:
...
async function loadOwnerTenantDashboard(env, startValue, endValue) {
  ...
  const [totalRow, hourlyResult, reasonResult, towerReasonResult, defaulterUnitResult] =
    await Promise.all([
      dbFirst(
        env,
  ...
  return {
    ...
    kpi: {
      ...
      defaulterRate: dashboardPct(defaulterCount, totalRegistration),
      defaulterCount,
      cardIssueRate: dashboardPct(cardIssueCount, totalRegistration),
      cardIssueCount,
    },
...
function validateRegistrationPayload(data) {
  ...
  const reason = cleanUpper(data.reason, 100);
  const reasonOther = cleanUpper(data.reasonOther, 160);

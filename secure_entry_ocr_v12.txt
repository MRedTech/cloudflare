export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // OCR WORKER VERSION: V12 — full 7-layer Gemini OCR gate
      ...
      const documentType = normalizeDocumentType(gateData.documentType);
      ...
      const extractedDocumentType = normalizeDocumentType(extractData.documentType);
      ...
      const finalBlocked =
        ...
        (documentType !== "UNKNOWN" && extractedDocumentType !== documentType);
      ...
      // PASS 4 — deterministic server-side identity validation.
      // Gemini reads the pixels; code decides whether the returned identifier is structurally valid.
      const idValidation = validateIdentityNumber(extractedDocumentType, idnum);
      ...
      if (!idValidation.valid) {
        return jsonResponse({
          ...
          message: "OCR could not verify the identity number format. Please enter the details manually."
        }, 200);
      }
      ...
      system_instruction: {
        parts: [{
          text:
            ...
            qualityHint +
            "Return THREE independent classifications: documentClass, documentType, and readability. " +
            ...
            "DOCUMENT TYPE rules: " +
            "MYKAD = Malaysian MyKad/MyPR/MyTentera-style national identity card when the identity-number layout is the 12-digit Malaysian NRIC pattern. " +
            "PASSPORT = a passport bio-data page or passport copy where the primary identity number is a passport number. " +
            "OTHER_ID = another clearly personal identity document such as driving licence, staff/student ID, foreign national ID, permit or similar. " +
            "UNKNOWN = use only for POSSIBLE_ID or NOT_ID, or when the subtype cannot be visually confirmed without inference. " +
            "Do not infer subtype from a guessed number. Use layout and clearly visible labels/structure only. " +
      ...
      generationConfig: {
        ...
        response_schema: {
          ...
          properties: {
            ...
            documentType: {
              type: "STRING",
              enum: ["MYKAD", "PASSPORT", "OTHER_ID", "UNKNOWN"]
            },
            readability: {
              type: "STRING",
          ...
          required: ["documentClass", "documentType", "readability"]
        }
      }
    })
  };
...
function buildExtractRequestOptions(apiKey, image, quality, documentType, nameEvidence, idEvidence) {
  return {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      ...
      system_instruction: {
        parts: [{
          text:
            ...
            "The document subtype gate returned " + documentType + ". " +
            "Field evidence is NAME=" + nameEvidence + " and ID=" + idEvidence + ". " +
            "Return documentType as MYKAD, PASSPORT, OTHER_ID, or UNKNOWN based only on the visible document. It must agree with the gated subtype unless the gated subtype was UNKNOWN. " +
...
function normalizeDocumentType(value) {
  const v = String(value || "").trim().toUpperCase();
  if (v === "MYKAD" || v === "PASSPORT" || v === "OTHER_ID" || v === "UNKNOWN") return v;
  return "UNKNOWN";
}

function validateIdentityNumber(documentType, value) {
  ...
  if (type === "MYKAD") {
    return /^\d{12}$/.test(compact)
      ...
      : { valid: false, reason: "MYKAD_MUST_BE_12_DIGITS" };
  }

  if (type === "PASSPORT") {
    // Secure Entry rule: passport must begin with a letter; remaining characters may be alphanumeric.
    return /^[A-Z][A-Z0-9]{4,19}$/.test(compact)
      ? { valid: true, reason: "OK" }
      : { valid: false, reason: "PASSPORT_FORMAT_INVALID" };
  }

  if (type === "OTHER_ID") {
    // Do not invent a country-specific rule for other IDs. Require a non-trivial visible identifier only.
    return /^[A-Z0-9][A-Z0-9]{3,19}$/.test(compact)
      ? { valid: true, reason: "OK" }
      : { valid: false, reason: "OTHER_ID_FORMAT_INVALID" };
  }

  return { valid: false, reason: "DOCUMENT_TYPE_UNKNOWN" };
}

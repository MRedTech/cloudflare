...
//   namePassport      // NORMAL + DEFAULTER TENANT; blank for DEFAULTER OWNER
//   mykadPassport     // NORMAL + DEFAULTER TENANT; blank for DEFAULTER OWNER
...
//   imageUrl          // NORMAL + DEFAULTER TENANT when a new ID photo is required
//
// IMPORTANT:
 ...
 * - Unit Number returns one retained identity reference per known TENANT for that unit.
...
function defaulterSearchRecordData(row) {
  ...
  return {
    ...
    photoLink: toText(row.drive_url),
  };
}
...
async function d1DefaulterUnitSearch(env, value) {
  ...
  // Unit Number is a tenant-reference lookup for DEFAULTER registration.
  // Use retained TENANT registrations from either NORMAL or DEFAULTER mode so
  // guards can select a known tenant by identity. Legacy DEFAULTER rows with
  // blank identity are intentionally excluded and remain reachable via WALK-IN.
  const result = await dbAll(
    env,
    `${FULL_RECORD_SELECT}
      WHERE unit_norm = ?
        AND created_at >= ?
        AND UPPER(TRIM(category)) = 'TENANT'
        AND COALESCE(TRIM(name_passport), '') <> ''
  ...
  // Show each tenant once using the latest retained registration for that ID.
  for (const row of rows) {
    const tenantKey = normKey(row.id_norm || row.mykad_passport);
    if (!tenantKey || seenTenantIds.has(tenantKey)) continue;
  ...
  return {
    exist: tenants.length > 0,
    mode: "DEFAULTER",
    searchType: "UNITNUMBER",
    unitNumber: cleanUpper(
      (tenants[0] && tenants[0].unitNumber) || unitNumber,
      40
    ),
    ...
    // Keep the old property present for compatibility, but the new frontend
    // intentionally renders the dedicated tenant list above instead of vehicles.
    records: [],
  };
}

async function d1DefaulterTenantRecordSearch(env, value) {
  const recordId = cleanInput(value, 100);
  if (!recordId) {
    return {
      exist: false,
      mode: "DEFAULTER",
      searchType: "TENANTRECORD",
      status: "NO_RECORD",
    };
  }

  const cutoffIso = retentionCutoffIso(env);
  const anchor = await dbFirst(
    env,
    `${FULL_RECORD_SELECT}
      WHERE id = ?
        AND created_at >= ?
  ...
  const unitNorm = normKey(anchor.unit_norm || anchor.unit_number);
  let idNorm = normKey(anchor.id_norm || anchor.mykad_passport);
  let identityRow = anchor;

  // Legacy DEFAULTER TENANT records were identity-free. For a REG.NUM lookup,
  // recover a retained TENANT identity only when the same unit + vehicle has one.
  if (!idNorm) {
    const regNorm = normKey(anchor.reg_norm || anchor.regnum);
    if (unitNorm && regNorm) {
      const recovered = await dbFirst(
        env,
        `${FULL_RECORD_SELECT}
          WHERE unit_norm = ?
            AND reg_norm = ?
      ...
      if (recovered) {
        identityRow = recovered;
        idNorm = normKey(recovered.id_norm || recovered.mykad_passport);
      }
    }
  }
  ...
  if (!idNorm || !unitNorm) {
    return {
      ...
      data: {
        id: toText(anchor.id),
        regnum: toText(anchor.regnum),
        contact: toText(anchor.contact),
        unitNumber: toText(anchor.unit_number),
        category: "TENANT",
        reason: "DEFAULTER",
        reasonOther: "",
        tower: towerFromUnitNumber(anchor.unit_number),
      },
    };
  }
  ...
  const data = {
    id: toText(latest.id || identityRow.id),
    createdAt: toText(latest.created_at || identityRow.created_at),
    namePassport: toText(latest.name_passport || identityRow.name_passport),
    mykadPassport: toText(latest.mykad_passport || identityRow.mykad_passport),
    regnum: toText(anchor.regnum || latest.regnum),
    contact: toText(latest.contact || anchor.contact),
    unitNumber: toText(latest.unit_number || anchor.unit_number),
    category: "TENANT",
    reason: "DEFAULTER",
    reasonOther: "",
    tower: towerFromUnitNumber(latest.unit_number || anchor.unit_number),
  };
...
async function d1Search(env, field, value, mode = "NORMAL") {
  ...
  if (searchMode === "DEFAULTER") {
    ...
    // Internal verification used after a guard selects a retained TENANT from
    // the DEFAULTER Unit Number / REG.NUM popup.
    if (["TENANTRECORD", "TENANTID"].includes(normalizedField)) {
      return d1DefaulterTenantRecordSearch(env, value);
    }

    // DEFAULTER user-facing search intentionally supports REG.NUM and UNIT NUMBER only.
    return {
      exist: false,
      mode: "DEFAULTER",
...
function validateRegistrationPayload(data) {
  ...
  const isDefaulterTenant = isDefaulterMode && category === "TENANT";
  const needsIdentity = !isDefaulterMode || isDefaulterTenant;

  // DEFAULTER OWNER remains identity-free. DEFAULTER TENANT follows the same
  // retained identity/photo-proof model as Secure Entry NORMAL registration.
  const namePassport = needsIdentity
    ? cleanUpper(data.namePassport, 120)
    : "";
  const mykadPassport = needsIdentity
  ...
  if (needsIdentity && !namePassport) {
    return { ok: false, message: "Name is required." };
  }
  if (needsIdentity && !mykadPassport) {
  ...
  // MyKad format validation applies whenever identity is required.
  if (needsIdentity) {
...
async function hasExistingTenantPhotoProof(env, idNorm, unitNorm) {
  if (!idNorm || !unitNorm) return false;
...
async function handleSubmit(data, env, ctx, cors) {
  ...
  const isDefaulterTenant =
    value.mode === "DEFAULTER" && value.category === "TENANT";

  // DEFAULTER OWNER remains identity/photo-free. DEFAULTER TENANT may upload
  // a new ID photo, or reuse retained proof for the same tenant + unit.
  const imageUrl = value.mode === "DEFAULTER" && !isDefaulterTenant
    ? ""
    : toText(data.imageUrl);

  // Keep the existing NORMAL proof rule unchanged.
  ...
  } else if (isDefaulterTenant) {
    if (!imageUrl) {
      const hasProof = await hasExistingTenantPhotoProof(env, idNorm, unitNorm);
      if (!hasProof) {
        return jsonResp(
          {
            success: false,
            message: "ID photo is required because no active previous Tenant photo was found.",
          },
          200,
          cors,
          { "Cache-Control": "no-store" }
      ...
      return jsonResp(
        {
          ...
          schemaVersion: 2,
          ownerDirectory: true,
          unitSearchMode: "NORMAL_OWNER_DETAILS__DEFAULTER_TENANT_REFERENCES",
...
Returns one retained TENANT identity per person for that unit in `tenants`, plus WALK-IN in the frontend.

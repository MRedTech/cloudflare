...
async function d1Search(env, field, value) {
  ...
  // STRICT: search ONLY the column decided by frontend autodetect
  let col = "";
  if (f === "REGNUM") col = "reg_norm";
  else if (f === "MYKADPASSPORT") col = "id_norm";
  else return { exist: false };

  const anyRow = await dbFirst(
    env,
    `SELECT id, created_at, name_passport, mykad_passport, regnum, contact, remark, unit_number
       FROM entries
      WHERE ${col} = ?
      ORDER BY created_at DESC
      LIMIT 1`,

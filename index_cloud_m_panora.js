 ...
 * ========================= */
function fmtRemark(remarkRaw) {
  return toUpper(remarkRaw);
}
    ...
    try {
      await dbRun(
        ...
        `INSERT INTO entries (
          ...
          unit_number, reason, reason_other,
          reg_norm, id_norm,
        ...
        toUpper(d.unitNumber), toUpper(d.reason), "", // reason_other unknown from sheet
        regNorm, idNorm,
        null, null,
...
async function syncToGoogle(env, record) {
  ...
  try {
    // Robust increment even if sync_attempts was NULL
    try {
...
async function d1Search(env, field, value) {
  ...
  for (const t of tryOrder) {
    anyRow = await dbFirst(
      env,
      `SELECT id, created_at, name_passport, mykad_passport, regnum, contact, remark, unit_number, reason, reason_other
         FROM entries
        WHERE ${t.col} = ?
        ORDER BY created_at DESC
        LIMIT 1`,
  ...
  return {
    ...
    data: {
      ...
      remark: fmtRemark(anyRow.remark),
      unitNumber: anyRow.unit_number || "",

      // Do NOT return reason to force guard to re-select every entry
      reason: "",
...
export default {
      ...
      try {
        ...
        try {
          ...
          if (imageUrl) {
            ...
            const prefix = toText(env.R2_PREFIX) || "m_panora";
          ...
          try {
            await dbRun(
              ...
              record.unit_number, record.reason, record.reason_other,
              regNorm, idNorm,
              imageKey, shaHex,
  ...
  async scheduled(event, env, ctx) {
    ...
    const rows = await dbAll(
      env,
      "SELECT id, created_at, client_txn_id, device_id, name_passport, mykad_passport, regnum, contact, remark, unit_number, reason, reason_other, image_key FROM entries WHERE sync_status IN ('PENDING','FAILED') AND sync_attempts < 5 ORDER BY created_at ASC LIMIT 20"
    );

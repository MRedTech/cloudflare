...
function ePassHtml(env, session, snapshot, activityLog = []) {
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && path === "/e-pass") {
      ...
      return new Response(ePassHtml(env, session, snapshot, activityLog), {
        status: 200,
        headers: {
          "Content-Type": "text/html; charset=UTF-8",
          "Cache-Control": "no-store",

export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // Final V9 veto:
      // - Never autofill if the extractor reports uncertainty.
      // - Never partial-autofill one field.
      // - Never return PII when evidence was not fully supported.
      if (confidence !== "CONFIDENT" || !name || !idnum) {
        return jsonResponse({
          success: false,
          manual: true,
          retake: false,
      ...
      system_instruction: {
        parts: [{
          text:
            ...
            "VISIBLE = the complete field is visually present and readable enough for reliable OCR. " +
            "PARTIAL = the complete field is still present and mostly visually supported, but a very small local defect affects about 1-2 characters (slight fade, wear, mild blur). There must still be one visually supported reading; no hidden section. " +
            "OCCLUDED = any important characters are physically covered, blocked by a finger/object/watermark/glare, cut off by crop/frame, outside the image, or otherwise not visible. " +
            "UNREADABLE = the field is present but too blurred/smeared/soft to read reliably. " +

            "HARD RULES: " +
            "1. A character that is physically hidden or cropped is OCCLUDED, never PARTIAL. " +
            "2. Do not use document patterns (for example MyKad number structure), common names, surrounding context, or likely spelling to fill missing evidence. " +
            "3. If several characters are not directly visible, choose OCCLUDED or UNREADABLE. " +
            "4. PARTIAL is only for a mostly visible field with a very small local defect, roughly 1-2 imperfect characters. " +
            "5. The question is visual evidence, not whether you think you can guess the answer."
        }]
      },
      contents: [{
        parts: [
      ...
      system_instruction: {
        parts: [{
          text:
            ...
            "Separate gates already confirmed that this is a personal identity document with overall quality " + quality + ". " +
            "Field evidence was classified as NAME=" + nameEvidence + " and ID=" + idEvidence + ". " +
            "You are allowed to extract ONLY because neither field was classified OCCLUDED or UNREADABLE. " +
            ...
            "HARD RULES: " +
            "1. Never invent or autocomplete an identity from familiarity, context, common names, number patterns, MyKad date/state structure, or likely spellings. " +
            "2. Never reconstruct characters that are hidden, covered, cropped, outside the image, or not directly visible. " +
            "3. If several characters are unclear, any expected character is not visually supported, or multiple readings are plausible, set confidence=UNCERTAIN. " +
            "4. If confidence=UNCERTAIN, return empty strings for BOTH name and idnum. " +
            "5. Do not mark UNCERTAIN merely because about 1-2 characters are slightly imperfect when the FULL field remains visually present and there is one supported reading. " +
            "6. Accuracy is more important than forcing an autofill."
        }]
      },
      contents: [{
        parts: [

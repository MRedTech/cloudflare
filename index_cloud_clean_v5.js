...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && (path === "/" || path === "/search")) {
      ...
      // Optional fallback to Apps Script search (if configured)
      if (!bodyObj && env.GAS_SEARCH_URL) {
...
const existingPhotoLink = toText(data.existingPhotoLink);
        const canReusePhoto = !imageUrl && isHttpUrl(existingPhotoLink);
        const driveUrlInit = canReusePhoto ? existingPhotoLink : "";
        const driveFileIdInit = canReusePhoto ? extractDriveFileId(existingPhotoLink) : "";
        let imageKey = null;
        let shaHex = null;
        let r2PutOk = false;

...
// - Required ENV: R2_PREFIX (example: visitor-sensory)
// ==================================================
...
async function d1PersonSearch(env, field, value) {
  ...
  // For identity searches, an active visit takes priority over older history.
  // Registration numbers are not unique to one person, so they remain latest-row searches.
  let row = null;
  if (isId) {
    row = await dbFirst(
      env,
  ...
  // Active records must include their details even before Drive sync completes,
  // so the frontend can prevent a second check-in immediately.
  if (activeCheckIn) {
    return {
      exist: true,
      hasHyperlink: !!photoLink,
...
async function handleCheckIn(data, env, ctx, cors) {
  ...
  // Hard identity rule: the same MyKad / Passport cannot have two active visits,
  // even if a different Visitor Pass Number is entered.
  ...
  try {
    ...
    try {
      ...
      // to register the same active MyKad / Passport.
      if (/UNIQUE constraint failed: entries\.id_norm/i.test(message)) {
        if (uploadedToR2 && imageKey) {
          try { await env.BUCKET.delete(imageKey); } catch (_) {}
        }

        const currentActive = await dbFirst(
          env,
          `SELECT id, name_passport, regnum, contact, visitor_pass_number,
                  unit_number, check_in_time, created_at
             FROM entries
        ...
        return jsonResp(
          {
            ...
            code: "VISITOR_ALREADY_CHECKED_IN",
            message: "This visitor already has an active check-in record. Please check out the previous record before registering again.",
            activeRecord: currentActive ? {
              recordId: currentActive.id || "",
              namePassport: currentActive.name_passport || "",
              regnum: currentActive.regnum || "",
              contact: currentActive.contact || "",
              visitorPassNumber: currentActive.visitor_pass_number || "",
              unitNumber: currentActive.unit_number || "",
              checkInTime: formatMalaysiaDateTime(currentActive.check_in_time || currentActive.created_at),
              status: "CHECKED_IN",
            } : null,
          },
          200,

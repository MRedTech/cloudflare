...
async function storeProofFromGasToD1(env, gasObj) {
  try {
...
async function cleanupOld(env, ctx) {
  ...
  if (!list.length) return { deletedRows: 0, deletedR2: 0, driveDeleteQueued: 0 };
  ...
  if (env.BUCKET && r2Keys.length) {
    for (const k of r2Keys) {
      try { await env.BUCKET.delete(k); deletedR2++; } catch (_) {}
    }
  }

  // Delete Drive files via Apps Script (best-effort, batched)
  let driveDeleteQueued = 0;
  if (driveIds.length && env.GAS_SYNC_URL && env.SYNC_TOKEN) {
    ...
    for (let i = 0; i < driveIds.length; i += chunkSize) {
      ...
      driveDeleteQueued += chunk.length;
    }
  }

  return { deletedRows: ids.length, deletedR2, driveDeleteQueued };
}

/** =========================
 * Trigger Apps Script cleanup for Sheet + Drive
 * Why:
 * - Apps Script cleanupByAge_ only runs on submit.
 * - If there is no new submit, Sheet/Drive won't auto-delete.
 * - Worker scheduled cleanup can call GAS action CLEANUP_AGE periodically.
 * ========================= */
async function triggerGasCleanupAge(env, ctx) {
  if (!env.GAS_SYNC_URL || !env.SYNC_TOKEN) return;
  const days = retentionDays(env);
  const payload = { token: env.SYNC_TOKEN, action: "CLEANUP_AGE", retentionDays: days };
  ctx.waitUntil(fetch(env.GAS_SYNC_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  }).catch(() => {}));
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && path === "/health") {
      return jsonResp({ ok: true, retentionDays: retentionDays(env) }, 200, cors);
    }

    // =========================
    // GET /admin/cleanup?token=...
    // Manual trigger to test retention without waiting for cron.
    // =========================
    if (request.method === "GET" && path === "/admin/cleanup") {
      const token = toText(url.searchParams.get("token"));
      if (!env.ADMIN_TOKEN || token !== toText(env.ADMIN_TOKEN)) {
        return textResp("Unauthorized", 401, cors);
      }

      const result = await cleanupOld(env, ctx);
      triggerGasCleanupAge(env, ctx);
    ...
    if (request.method === "GET" && (path === "/" || path === "/search")) {
      ...
      if (gasBase && (!bodyObj || bodyObj.exist === false)) {
        ...
        if (res.ok && parsed && env.DB) {
          try { if (parsed.data) { parsed.data.tower = ""; parsed.data.reason = ""; parsed.data.reasonOther = ""; } } catch (_) {}
          ctx.waitUntil(storeProofFromGasToD1(env, parsed));
        }
  ...
  async scheduled(event, env, ctx) {
    // NOTE: D1/R2 do NOT auto-expire by themselves.
    // Retention cleanup runs ONLY when a Cron Trigger is configured for this Worker.
    if (!env.DB) {
      // Still attempt Sheet/Drive cleanup (best-effort)
      triggerGasCleanupAge(env, ctx);
      return;
    }
    ...
    // 3) Trigger Sheet/Drive cleanup (so it runs even when there is no new submit)
    triggerGasCleanupAge(env, ctx);
  }
};

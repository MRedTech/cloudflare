...
async function syncToGoogle(env, record) {
  ...
  // NEW-SEC-04:
  // If this record has an R2 image, the protected image handoff must be fully
  // configured. Do not silently sync a photo-bearing row without its Drive copy.
  if (record.image_key && (!env.PUBLIC_BASE_URL || !env.IMAGE_VIEW_TOKEN)) {
    await markSyncFailed(
      env,
      record.id,
      leaseToken,
      !env.PUBLIC_BASE_URL ? "PUBLIC_BASE_URL is missing" : "IMAGE_VIEW_TOKEN is missing"
    );
    return {
      ok: false,
      claimed: true,
      message: !env.PUBLIC_BASE_URL ? "PUBLIC_BASE_URL is missing" : "IMAGE_VIEW_TOKEN is missing"
    };
  }

  // Token is intentionally NOT placed in the URL. GAS supplies the bearer
  // credential from Apps Script Script Properties when fetching this URL.
  const imageViewUrl =
    record.image_key
      ? `${String(env.PUBLIC_BASE_URL).replace(/\/+$/, "")}/image?id=${encodeURIComponent(record.id)}`
      : "";
...
async function confirmDriveCleanup(env, rows) {
  ...
  if (!env.GAS_SYNC_URL || !env.SYNC_TOKEN) return confirmed;
...
async function cleanupOld(env, ctx, cutoffIsoOverride) {
  ...
  for (let b = 0; b < maxBatches; b++) {
    ...
    if (!cursorCreatedAt) {
      batch = await dbAll(
        env,
        `SELECT id, created_at, image_key, drive_file_id
           FROM entries
          WHERE created_at < ?
            AND (
              sync_lease_until IS NULL
    ...
    const driveConfirmed = await confirmDriveCleanup(env, driveCandidates);

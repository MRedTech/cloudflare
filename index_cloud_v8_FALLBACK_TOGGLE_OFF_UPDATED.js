...
// - GET / or /search: FAST search from D1 only by default (optional GAS fallback via ENABLE_GAS_FALLBACK=true)
...
function gasFallbackEnabled(env) {
  const v = toText(env && env.ENABLE_GAS_FALLBACK).toLowerCase();
  if (!v) return false; // default OFF
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && path === "/health") {
      return jsonResp({ ok: true, gasFallback: gasFallbackEnabled(env), retentionDays: retentionDays(env) }, 200, cors);
    }
    ...
    if (request.method === "GET" && (path === "/" || path === "/search")) {
      ...
      // Optional fallback to Apps Script search (default OFF after D1 retention window is complete)
      ...
      if (gasFallbackEnabled(env) && gasBase && (!bodyObj || bodyObj.exist === false)) {
        ...
        if (res.ok && parsed && gasIsMismatch(field, value, parsed)) {
          ...
          // Use retry only if it is valid JSON and not an error response
        ...
        // If GAS returned ACTIVE with photoLink, store proof into D1 for next searches
        if (res.ok && parsed && env.DB) {
          ...
          // Only store PROOF when returned key matches searched key (prevents pollution)
      ...
      // Return D1 result only (fallback OFF by default)

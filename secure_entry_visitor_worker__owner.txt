...
function buildParkingChargeAllocation(
  activityLog,
  freeSeconds,
  blockSeconds,
  blockFeeCents,
  ...
  const activities = Array.isArray(activityLog) ? activityLog.slice() : [];

  // Allocation must follow actual parking time, so work from the oldest
  // interval to the latest even though the PDF activity log is latest first.
  activities.sort((left, right) => {
    const leftMs = parseIsoMs(left && left.checkInTime);
    const rightMs = parseIsoMs(right && right.checkInTime);
  ...
  for (const item of activities) {
    const unit = cleanUpper(item && item.unitNumber, 40);
    const key = normKey(unit);
    if (!key || seenUnits.has(key)) continue;
    seenUnits.add(key);
    unitsVisited.push(unit);
  }

  const fallback = cleanUpper(fallbackUnit, 40);
  if (!unitsVisited.length && fallback) unitsVisited.push(fallback);

  const safeFreeSeconds = Math.max(0, Math.floor(Number(freeSeconds) || 0));
  const safeBlockSeconds = Math.max(1, Math.floor(Number(blockSeconds) || 1));
  const safeFeeCents = Math.max(0, Math.floor(Number(blockFeeCents) || 0));
  const safeChargeBlocks = Math.max(0, Math.floor(Number(chargeBlocks) || 0));
  const defaultUnit = fallback || unitsVisited[unitsVisited.length - 1] || "";

  const chargeAllocation = [];

  for (let periodNumber = 1; periodNumber <= safeChargeBlocks; periodNumber++) {
    // The charge starts at the first second beyond each threshold:
    // 24h, 48h, 72h and so on.
    const thresholdSeconds =
      safeFreeSeconds + ((periodNumber - 1) * safeBlockSeconds);
    ...
    for (const item of activities) {
      const durationSeconds = Math.max(
        0,
        Math.floor(Number(item && item.durationSeconds) || 0)
      );
      const intervalEndSeconds = accumulatedSeconds + durationSeconds;

      // Strictly greater is intentional. If an interval ends exactly at the
      // threshold, the next interval owns the additional charge period.
      if (intervalEndSeconds > thresholdSeconds) {
        allocatedUnit = cleanUpper(item && item.unitNumber, 40) || defaultUnit;
        break;
      }

      accumulatedSeconds = intervalEndSeconds;
    }

    chargeAllocation.push({
      periodNumber,
      label: `ADDITIONAL 24-HOUR PERIOD ${periodNumber}`,
      thresholdSeconds,
      unitNumber: allocatedUnit || defaultUnit || "-",
      amountCents: safeFeeCents,
      amount: formatRinggit(safeFeeCents),
    });
  }

  return {
    multiUnit: unitsVisited.length > 1,
    unitsVisited,
    chargeAllocation,
  };
}
...
async function sendParkingNoticeToGoogle(env, noticeId) {
  ...
  for (let attempt = 1; attempt <= tries; attempt++) {
    ...
    const allocation = buildParkingChargeAllocation(
      activityLog,
      Number(notice.free_seconds || parkingFreeSeconds(env)),
      parkingBlockSeconds(env),
      Number(notice.rate_cents || parkingBlockFeeCents(env)),
      Number(notice.charge_blocks || 0),
      notice.unit_number
    );
    ...
    const payload = {
      ...
      unitsVisited: allocation.unitsVisited,
      multiUnit: allocation.multiUnit,
      chargeAllocation: allocation.chargeAllocation,
...
async function loadUnitOwnerDetails(env, unitNorm) {
  try {
    const result = await dbAll(
      env,
    ...
    const owners = rows
      .map((row) => ({
        name: toText(row.owner_name),
        order: Math.max(1, Math.floor(Number(row.owner_order) || 1)),
      }))
    ...
    return {
      ...
      unitNumber: toText(rows[0] && rows[0].unit_number),
    ...
    // The same Worker template can still be used at a site where the owner
    // directory table has not been created yet. Visitor search remains active.
...
async function d1UnitSearch(env, value) {
  ...
  const [result, ownerDetails] = await Promise.all([
    ...
    loadUnitOwnerDetails(env, unitNorm),
  ]);

  const rows = result && Array.isArray(result.results) ? result.results : [];
  const records = await Promise.all(rows.map((row) => mapSearchRecord(row, env)));
  const displayUnitNumber =
    toText(ownerDetails.unitNumber) ||
    toText(records[0] && records[0].unitNumber) ||
    cleanUpper(value, 40);

  return {
    exist: records.length > 0 || ownerDetails.unitFound,
      ...
      return jsonResp(
        {
          ...
          parkingNotice: true,
          multiUnitChargeAllocation: true,

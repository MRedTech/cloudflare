 ...
 * - Created only when the final checkout charge is greater than RM0.00.
...
async function prepareParkingNoticeForCheckout(env, current, session, snapshot, checkOutIso) {
  if (!snapshot || Number(snapshot.currentChargeCents || 0) <= 0) {
    return { notice: null, shouldSend: false };
  }
  ...
  if (!existing) {
    ...
    return {
      notice: await loadParkingNoticeById(env, noticeId),
      shouldSend: true,
    };
  }

  const changed =
    toText(existing.final_check_out_at) !== toText(checkOutIso) ||
  ...
  if (changed) {
    ...
    return {
      notice: await loadParkingNoticeById(env, existing.id),
      shouldSend: true,
    };
  }
  ...
  return {
    notice: existing,
    shouldSend: toUpper(existing.notice_status) === "FAILED",
  };
}
...
async function retryPendingParkingNotices(env, ctx, overrideLimit) {
  ...
  const rows = await dbAll(
    ...
    `SELECT id
       FROM parking_notices
      WHERE notice_status IN ('PENDING', 'FAILED')
...
function ePassHtml(env, session, snapshot, activityLog = [], parkingNotice = null) {
  ...
  const noticeMessage = snapshot.status === "ACTIVE"
    ? "A PARKING CHARGE NOTICE WILL BE SUBMITTED TO THE MANAGEMENT OFFICE AFTER CHECK-OUT."
    : noticeStatus === "SENT"
      ? "THE PARKING CHARGE NOTICE HAS BEEN SUBMITTED TO THE MANAGEMENT OFFICE."
      : noticeStatus === "FAILED"
        ? "NOTICE SUBMISSION IS PENDING SYSTEM RETRY."
        : "THE PARKING CHARGE NOTICE IS BEING SUBMITTED TO THE MANAGEMENT OFFICE.";

  const chargeNoticeHtml = `<section id="chargeNotice" class="charge-notice"${snapshot.currentChargeCents > 0 ? "" : ' style="display:none"'}>\
    <h3>OVERNIGHT PARKING CHARGE NOTICE</h3>\
    <div class="charge-notice-row"><span>${snapshot.status === "ACTIVE" ? "CURRENT CHARGE" : "TOTAL CHARGE"}</span><b id="noticeCharge">${htmlEscape(snapshot.currentCharge)}</b></div>\
...
async function handleCheckOut(data, env, ctx, cors) {
  ...
  const finalizedSession = await finalizeParkingForEntry(env, current, checkOutTime);
  ...
  // Checkout must remain successful even if PDF/email delivery is temporarily unavailable.
  // The notice is saved as PENDING/FAILED and retried by the scheduled Worker.
  if (mapped && mapped.parking && mapped.parking.currentChargeCents > 0 && finalizedSession) {
    try {
      const prepared = await prepareParkingNoticeForCheckout(
        env,
        current,
        finalizedSession,
        mapped.parking,
      ...
      parkingNotice = mapParkingNotice(prepared.notice);
      if (prepared.shouldSend && prepared.notice) {
        ctx.waitUntil(sendParkingNoticeToGoogle(env, prepared.notice.id));
      }
    } catch (error) {
      parkingNotice = {
        required: true,
        noticeNumber: "",
        status: "FAILED",
        statusLabel: "PENDING SYSTEM RETRY",
        totalDurationSeconds: mapped.parking.durationSeconds,
        totalDuration: mapped.parking.duration,
        chargeBlocks: mapped.parking.chargeBlocks,
        rateCents: mapped.parking.blockFeeCents,
        rate: mapped.parking.blockFee,
        totalChargeCents: mapped.parking.currentChargeCents,
        totalCharge: mapped.parking.currentCharge,
        finalCheckOutAt: checkOutTime,
      };
      console.error("Parking notice preparation failed:", cleanSyncError(error));
    }
  }
...
export default {
  ...
  async scheduled(event, env, ctx) {
    ...
    ctx.waitUntil(retryPendingParkingNotices(env, ctx).catch(() => {}));

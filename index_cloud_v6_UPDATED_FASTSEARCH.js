...
async function d1Search(env, field, value) {
  ...
  // One-query-per-key (anyRow + latest proof drive_url) for max speed
  for (const t of tryOrder) {
    const row = await dbFirst(
      env,
      `SELECT
          id, created_at,
          name_passport, mykad_passport, regnum, contact, remark, unit_number, tower, reason, reason_other,
          (
            SELECT drive_url
              FROM entries p
             WHERE p.${t.col} = ?
               AND p.drive_url IS NOT NULL
               AND p.drive_url <> ''
    ...
    const proofUrl = toText(row.proof_drive_url);
    ...
    return {
      ...
      hasHyperlink: true,
      data: {
        namePassport: row.name_passport || "",
        mykadPassport: row.mykad_passport || "",
        regnum: row.regnum || "",
        contact: row.contact || "",
        remark: fmtRemark(row.remark, row.unit_number),
        tower: "", // Tower intentionally not returned (force user to update)
        reason: fmtReason(row.reason, row.reason_other),
        photoLink: proofUrl,
      },
    };
  }
    ...
    if (request.method === "GET" && (path === "/" || path === "/search")) {
      ...
      if (gasBase && (!bodyObj || bodyObj.exist === false)) {
        ...
        if (res.ok && parsed && env.DB) {
          try { if (parsed.data) parsed.data.tower = ""; } catch (_) {}
...
CREATE INDEX IF NOT EXISTS idx_entries_reg_norm_created ON entries(reg_norm, created_at);
CREATE INDEX IF NOT EXISTS idx_entries_id_norm_created  ON entries(id_norm, created_at);
CREATE INDEX IF NOT EXISTS idx_entries_reg_norm_drive_created ON entries(reg_norm, drive_url, created_at);
CREATE INDEX IF NOT EXISTS idx_entries_id_norm_drive_created  ON entries(id_norm, drive_url, created_at);
